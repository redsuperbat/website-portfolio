<template>
    <nav>
      <!-- Desktop Mode -->
        <v-app-bar fixed flat class="transparent hidden-sm-and-down">
            <v-toolbar-title style="cursor:pointer" class="clickable">
                <div @click="setPage('home')">
                <span style="font-weight:lighter" :class="{'white-background' : textColor}">Max</span>
        ...
        >
          <v-list>
            <v-list-item v-for="(item, name) in header" :key="name">
                <v-btn @click="scroll(name); openDrawer();" depressed color="transparent">
                    <span v-if="name===currentPage">|</span><h4 :class="name===currentPage?'selected':'clickable'" >{{item}}</h4>
                </v-btn>
            </v-list-item>
          </v-list>
